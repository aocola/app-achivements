...
function getHighestMedalFromSet(currentMedalTypes: Set<string>): MedalType {
    ...
    if (validMedals.length === 0) {
        return MEDALS.MADERA; // Valor mínimo permitido
    }
...
// getStyle.js
function getStyle(tipo: string): string {
    const medalStyles: { [key in MedalType]: string } = {
      MADERA: 'medalMadera',
      HIERRO: 'medalHierro',
      BRONCE: 'medalBronce',
      PLATA: 'medalPlata',
      ...
      INMORTAL: 'medalInmortal',
      RADIANTE: 'medalRadiante',
    };
  
    return medalStyles[tipo as MedalType] || 'defaultMedal';
  }
